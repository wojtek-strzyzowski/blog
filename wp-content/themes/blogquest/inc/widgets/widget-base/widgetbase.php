 ...
 * @extends  WP_Widget
 */
abstract class BlogQuest_Widget_Base extends WP_Widget {
    ...
    public function __construct() {
        ...
        $this->attrs = apply_filters(
            'blogquest_widget_base_attr_arr',
            array(
                'type'          => 'text',
                'class'         => '',
                'wrapper_class' => '',
        ...
        add_action( 'admin_enqueue_scripts', array( $this, 'blogquest_widget_scripts' ) );
    ...
    public function update( $new_instance, $old_instance ) {
        ...
        foreach ( $this->settings as $key => $setting ) {
             ...
             */
            $instance[ $key ] = apply_filters( 'blogquest_widget_settings_sanitize', $instance[ $key ], $new_instance, $key, $setting );
        }
        ...
        foreach ( $this->settings as $key => $setting ) {
            ...
            switch ( $setting['type'] ) {
                            ...
                            <div class="image-field">
                                ...
                                <p>
                                    ...
                                    <button type="button" class="upload_image_button button" data-uploader-title-txt="<?php esc_attr_e( 'Use Image', 'blogquest' ); ?>" data-uploader-btn-txt="<?php esc_attr_e( 'Choose an Image', 'blogquest' ); ?>">
                                        <?php esc_html_e( 'Upload/Add image', 'blogquest' ); ?>
                                    </button>
                                    <button type="button" class="remove_image_button button" style="<?php echo esc_attr( $remove_button_style ); ?>"><?php esc_html_e( 'Remove image', 'blogquest' ); ?></button>
                                </p>
                            </div>
                        </div>
                        <?php
                ...
                case 'message':
                    ...
                    ?>
                    <div class="<?php echo esc_attr( $attr['wrapper_class'] ); ?>">
                        <div class="widefat field-message uf-custom-field-message <?php echo esc_attr( $attr['class'] ); ?>" style="<?php echo esc_attr( $css ); ?>">
                ...
                default:
                    do_action( 'blogquest_widget_field_' . $setting['type'], $key, $value, $setting, $instance );
                    break;
            }
        }
    }
     ...
     * @return void
     */
    public function blogquest_widget_scripts( $hook ) {
        if ( 'widgets.php' === $hook ) {
            wp_enqueue_style( 'wp-color-picker' );
            wp_enqueue_media();
            wp_enqueue_style( 'blogquest_widgets_css', get_template_directory_uri() . '/inc/widgets/widget-base/css/style.css' );
            wp_enqueue_script( 'blogquest_widgets_js', get_template_directory_uri() . '/inc/widgets/widget-base/js/script.js', array( 'jquery', 'wp-color-picker' ), '1.0.0', true );
        }
    }
